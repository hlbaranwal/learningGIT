...
public class PartyServices {
    ...
    public static Map<String, Object> findAllPerson(DispatchContext dctx, Map<String, ? extends Object> context) {
        Delegator delegator = dctx.getDelegator();
        Locale locale = (Locale) context.get("locale");
        Map<String,Object>result = ServiceUtil.returnSuccess();
        try{
        List<GenericValue> results = EntityQuery.use(delegator).select("partyId","firstName","lastName").from("Person").queryList();
        Debug.log(results.get(results.size()-1).toString());
        result.put("mylist",results);
        }catch(Exception e){
